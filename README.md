...
> **Enhanced fork of Sieve with OR query support and type-safe query building**

Sieve.Plus is a simple, clean, and extensible framework for .NET that **adds sorting, filtering, and pagination functionality out of the box**.
...
- ✨ **OR Query Support** - Use `||` operator for flexible filtering (e.g., `filters=Title@=awesome || LikeCount>100`)
- 🔒 **Type-Safe Query Builder** - Build queries with compile-time safety (separate `Sieve.QueryBuilder` package)
...
Sieve.Plus maintains full backward compatibility with the original Sieve while adding new features like OR query support and type-safe query building. See the [NOTICE](NOTICE) file for details on modifications.

[Get Sieve.Plus on NuGet](https://www.nuget.org/packages/Sieve.Plus/)
...
### OR Queries (New Feature)

You can now use OR logic across different filter conditions using `||` (double pipe):

```curl
GET /GetPosts

# Simple OR: Get posts where title contains "awesome" OR like count is greater than 100
?filters=Title@=awesome || LikeCount>100

# Multiple OR groups: Get posts with title A, B, or C
?filters=Title==A || Title==B || Title==C

# Combining AND and OR: Get posts where (title contains "awesome" OR likes > 100) AND status is active
?filters=Title@=awesome || LikeCount>100, Status==Active

# Complex groups: Get posts where (title is "awesome" AND likes > 100) OR (title is "great" AND comments > 50)
?filters=Title@=awesome, LikeCount>100 || Title@=great, CommentCount>50

```

This is particularly useful for business cases where you need flexible filtering such as:
- Show all VIP customers OR customers with orders over $1000
- Display urgent tickets OR tickets assigned to current user
- Filter products that are on sale OR have high ratings
...
* `filters` is a comma-delimited list of `{Name}{Operator}{Value}` where
    ...
    * **OR queries across different filter conditions**: Use `||` (double pipe) to separate independent filter groups that should be combined with OR logic
        * Example: `Title@=awesome || LikeCount>100` returns posts where the title contains "awesome" **OR** the like count is greater than 100
        * You can combine multiple OR groups: `Title==A || Title==B || Title==C`
        * Mix AND and OR logic: `Title@=awesome || LikeCount>100, CommentCount>5` means `(Title contains "awesome" OR LikeCount > 100) AND CommentCount > 5`
        * Complex groups: `Title@=awesome, LikeCount>100 || Title@=great, CommentCount>50` means `(Title contains "awesome" AND LikeCount > 100) OR (Title contains "great" AND CommentCount > 50)`
...
Sieve.Plus is licensed under Apache 2.0. Contributions are highly appreciated!
